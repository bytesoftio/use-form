...
describe("useForm", () => {
  ...
  it("hooks all form state", () => {
    ...
    expect(target().text()).toBe(`{"foo":"bar"},{"field":["error"]},true,false,[],[]`)

    act(() => form.submitted.set(true))

    expect(changes).toBe(4)
    expect(target().text()).toBe(`{"foo":"bar"},{"field":["error"]},true,true,[],[]`)

    act(() => form.dirtyFields.add("field1"))

    expect(changes).toBe(5)
    expect(target().text()).toBe(`{"foo":"bar"},{"field":["error"]},true,true,["field1"],[]`)
